      ...
      <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-16">
          {features.map((feature, i) => (
            <div
              key={feature.title}
              className={`reveal reveal-delay-${(i % 4) + 1} flex gap-5`}
            >
              {/* Icon container */}
              <div className="shrink-0 w-14 h-14 rounded-[var(--radius-md)] bg-[var(--accent-light)] flex items-center justify-center text-[var(--accent)]">
                {feature.icon}
              </div>
              <div>
                <h3 className="font-[var(--font-heading)] text-xl mb-2 text-[var(--text-primary)]">
                  {feature.title}
                </h3>
